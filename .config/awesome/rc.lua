...
function naughty.config.notify_callback(args)
	...
	awful.spawn("mpv --no-video --quiet " .. home .. "/.config/awesome/long-pop.wav")
	return args
end
...
awful.util.tagnames = tagnames.pick_a_tagnames_style("named")
...
-- autostart programs
local home = os.getenv("HOME")
awful.spawn.single_instance(string.format("%s/.config/autostart-apps-wm/autostart.sh", home), awful.rules.rules)
awful.spawn.single_instance(string.format("%s/.config/awesome/autostart.sh", home), awful.rules.rules)
